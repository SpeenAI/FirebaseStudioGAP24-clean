...
export const AbtretungForm: React.FC<AbtretungFormProps> = ({
  isOpen,
  onClose,
  onSave,
  caseData,
  ...
  useEffect(() => {
    ...
    const fuelleOrtUndDatum = (ortName?: string) => {
      const wert =
        ortName && ortName.length > 0
          ? `${ortName}, den ${datumString}`
          : `den ${datumString}`;
      setFormData((vorher) => ({
        ...vorher,
        ortDatumUnterschriftAbtretung: wert,
        ortDatumUnterschriftDatenschutz: wert,
        ortDatumUnterschriftWerkvertrag: wert,
        ortDatumUnterschriftSachverstaendiger: wert,
        ortDatumUnterschriftWiderruf1: wert,
        ortDatumUnterschriftWiderruf2: wert,
      }));
    };
      ...
      navigator.geolocation.getCurrentPosition(
        async ({ coords }) => {
          try {
            ...
            const ort = daten.city || daten.principalSubdivision || daten.locality;
            fuelleOrtUndDatum(ort);
          } catch (fehler) {
            // Bei Fehler einfach Datum belassen
          }
        },
        (fehler) => {
          // Bei Verweigerung oder Fehler: keine Änderung
        }
      );
    }
  }, []);
  ...
  const handleGeneratePdf = async () => {
    ...
    if (!contentElement) {
      toast({
        title: 'Fehler',
        description: 'Formularinhalt nicht gefunden.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const pdfDokument = await PDFDocument.create();
      const pngBild = await pdfDokument.embedPng(imgData);
      const seitenGroesse = PageSizes.A4;
      let verbleibendeHoehe = canvas.height;
      let yVersatz = 0;

      while (verbleibendeHoehe > 0) {
        const seite = pdfDokument.addPage(seitenGroesse);
        const { width: seitenBreite, height: seitenHoehe } = seite.getSize();
        const skala = seitenBreite / canvas.width;
        const teilausschnittHoehe = seitenHoehe / skala;

        seite.drawImage(pngBild, {
          x: 0,
          y: seitenHoehe - teilausschnittHoehe * skala - yVersatz * skala,
          width: seitenBreite,
          height: canvas.height *
            skala,
        });

        yVersatz += teilausschnittHoehe;
        verbleibendeHoehe -= teilausschnittHoehe;
      }

      const pdfBytes = await pdfDokument.save();
      const pdfDatei = new File(
        [pdfBytes],
        `Abtretung_${caseData.caseNumber}.pdf`,
        { type: 'application/pdf' }
      );

      onSave(pdfDatei);

      const url = URL.createObjectURL(pdfDatei);
      const aElement = document.createElement('a');
      aElement.href = url;
      aElement.download = pdfDatei.name;
      aElement.click();
    ...
    <div className="grid grid-cols-2 gap-4 items-end mt-4">
      <div className="space-y-2">
        ...
        <div className="border rounded-md bg-white">
          <SignatureCanvas canvasProps={{ className: 'w-full h-[100px]' }} ref={canvasRef} />
        </div>
        <Button
          variant="link"
          size="sm"
        ...
        <div ref={printRef} className="p-8 bg-white text-black">
          ...
          <LegalTextBox>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody>
                {honorarTableData.map((row, index) => (
                  <tr key={index}>
          ...
          <SignatureSection
            canvasRef={sigRefs.werkvertrag}
            id="ortDatumUnterschriftWerkvertrag"
            label="Unterschrift Auftraggeber"
          />
          <SignatureSection
            canvasRef={sigRefs.werkvertragSV}
            id="ortDatumUnterschriftSachverstaendiger"
            label="Unterschrift Sachverständiger"
          />

          <SectionTitle>Widerrufsbelehrung</SectionTitle>
          <LegalTextBox>{textWiderruf1}</LegalTextBox>
          <SignatureSection
            canvasRef={sigRefs.widerruf1}
            id="ortDatumUnterschriftWiderruf1"
            label="Unterschrift Auftraggeber"
          />
          <LegalTextBox>{textWiderruf2}</LegalTextBox>
          <SignatureSection
            canvasRef={sigRefs.widerruf2}
            id="ortDatumUnterschriftWiderruf2"
            label="Unterschrift Auftraggeber"
          />
        </div>
